...
if(preg_match("#^[a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4}$#", $email) AND !empty($_POST['password']) AND !empty($_POST['passwordConfirm']) AND $_POST['passwordConfirm']==$_POST['password']) {
    $password =  password_hash(htmlspecialchars($_POST['password']), PASSWORD_DEFAULT);
    $response = $db->query("SELECT email FROM users WHERE email='$email'");
    if ($response->fetch(PDO::FETCH_ASSOC)) {
        header('Location:index.php');
    } else {
        $response=$db->prepare("INSERT INTO users (password, email, date_created) VALUES (:password, :email, NOW())");
